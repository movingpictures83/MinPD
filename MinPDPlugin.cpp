...
int MinPDPlugin::BitCriteria(int Num, int Bit){/* Is bit set in Num */
	int i;
	int PowBit;
...
int MinPDPlugin::TwoCrossovers(int Num, int MaxBit, int *Begin, int *End){/* Is bit set in Num */
	int i;
	int Start,Stop;
	Stop=-1;
	Start=-1;
...
char** MinPDPlugin::MatrixInit(int s, int dim1, int dim2)
{
	char  **matrix  = NULL;
	int   i;
...
/*********************************************************************/
double MinPDPlugin::TN93distance(int pT, int pC, int pG, int pA, int p1, int p2, int gaps,int matches, int al_len, double alpha, int s, int t)
{
...
/*  																  */
/**********************************************************************/
int MinPDPlugin::DistOnly(Fasta **seqs, double **dist, double ***dist_frags, int maxdim, int n, double alpha, int Fr_Count)
{
...
/*  																  */
/**********************************************************************/
int MinPDPlugin::Align_Dist(Fasta **seqs, double **dist, double ***dist_frags, int maxdim, int n,  double GOP, double  GEP, double match, double mismatch,double alpha, int Fr_Count, char* file3)
{
	...
	int			offset,m;
	if ((fp2 = (FILE *) fopen(file3,"w")) == NULL) {
...
/*  																  */
/**********************************************************************/
int MinPDPlugin::SaveMinResults(Mintaxa **minresults, int *resno, int a, int c, double dist)
{
...
/*  																  */
/**********************************************************************/
void MinPDPlugin::OptionalRecPrintout(FILE *fp4,  RecRes *recRes,Fasta **seqs,int *Frag_Seq,FDisNode (*recSolutions)[3][2], int i, int l, int r, int p, int q, int f, int index, int last, int BKP, int Fr_Count,int align,int fr_size)
{
	double d_min;
	d_min=INF9999;
...
/* 							FillArrayInOrder					      */
/**********************************************************************/
void MinPDPlugin::FillArrayInOrder(int *arrayInOrder, double ***dist_frags, int  *Frag_Seq, int s, int k, int rec_idx)
{
	int t,j,p,q;
...
/*  																  */
/**********************************************************************/
BKPNode * MinPDPlugin::GetRecSolutionsII(FILE *fp4,  double ***dist_frags, Fasta **seqs, int  *Frag_Seq, int rec_idx, int s, int Fr_Count, int min_seq, int fr_size, double min_seq_dist, int align)
{
...
/*  																  */
/**********************************************************************/
int MinPDPlugin::PickSeqofLargerAvgDis(double ***dist_frags, int k_idx, int f_idx, int min_idx,  int s, int Fr_Count, int k, int f)
{
...
/* 						Print Fragments							      */
/**********************************************************************/
void MinPDPlugin::PrintFragments(FILE *fp3, int  *Frag_Seq, Fasta **seqs, double **dist, double ***dist_frags, int Fr_Count, int i, int s, int align )
{
	int t,k;
...
BKPNode* MinPDPlugin::Check4Recombination(FILE *fp3, FILE *fp4, int  *Frag_Seq, int  *MinCount, Fasta **seqs, double **dist, double ***dist_frags, int StartAncestorSearch, int s, int Fr_Count, double min_d, int min_idx, int fr_size, int align)
{
	int k,t,min_seq, i;
...
/*  																  */
/**********************************************************************/
BKPNode* MinPDPlugin::SaveResultsinBKPLinkList(int  *Frag_Seq, Fasta **seqs, double ***dist_frags, int Fr_Count,int i, int align, int s, int maxdim )
{
...
/*  																  */
/**********************************************************************/
int MinPDPlugin::GetMinDist(Mintaxa **minresults, Fasta **seqs, double **dist, double ***dist_frags, int maxdim, int n, char *File1, char *File2, double mutrate, int Fr_Count, int *resno, int align)
{
...
void MinPDPlugin::BuildPartialNJTrees(Fasta **seqs, Mintaxa **minresults, int resno, double **dist, char *File1, int n)
{
	FILE	*fp1;
	int r, ch, i,j,a, w;
	int max_w=0;
...
void MinPDPlugin::input(std::string infile)
{
	...
	fn = std::string(PluginManager::prefix())+"/"+fn;
	std::cout << fn << std::endl;
	filename = fn.c_str();
	...
	std::string data;
	ifile >> data;
	align=atoi(data.c_str());
	ifile >> data;
	Fr_Count=atoi(data.c_str());/* 4 or 8 for  recombination detection*/
...
void MinPDPlugin::output(std::string outfile) {
	std::string f1 = outfile+".out.txt";
	std::string f2 = outfile+".d1.txt";
        outputfile1 = f1.c_str();
	outputfile2 = f2.c_str();
	std::string f3 = outfile+".align.txt";
	char* outputfile3 = f3.c_str();

	if(align){
		printf("Pairwise alignments... ");
		res_no= Align_Dist(seqs,dist, dist_frags,  maxdim, n, GOP, GEP, match, mismatch, alpha, Fr_Count, outputfile3);
	}
	else{
...
void MinPDPlugin::heapify_mintaxa ( Mintaxa **list , int newnode )
{
	int done=0, dad;
	Mintaxa *temp;
...
/* Heap sort*/
void MinPDPlugin::heapsort_mintaxa ( Mintaxa **list, int last )
{
	int i;
	Mintaxa *temp;
	while(last > 0 )
...
/* Convert any random array to heap array*/
void MinPDPlugin::heapify_Fasta ( Fasta **list , int newnode )
{
	int done=0, dad;
	Fasta *temp;
...
/* Heap sort*/
void MinPDPlugin::heapsort_Fasta ( Fasta **list, int last )
{
	int i;
	Fasta *temp;
	while(last > 0 )
...
/*  																  */
/**********************************************************************/
void MinPDPlugin::AddTipOrNode(int *w,int *p,int *max_w, int chidx, double brlen,TNode **NodeStorage, int Join)
{
	int i;
	TNode *NewNode;
...
/*  																  */
/**********************************************************************/
void MinPDPlugin::PrintTree(FILE *fv, TNode *node,Fasta **seqs)
{
	if (node->nodeNo>=1000) {
		fputc('(', fv);				
...
void MinPDPlugin::NJTree(TNode **NodeStorage, double ** DistMatrix, int UB, int OutgroupIn0, int *seqIds, int *max_w, int *w)
{
	int i,j, x,y,p,q;
	int internalnodes;
	double	*div,d,dv;
...
PluginProxy<MinPDPlugin> MinPDPluginProxy = PluginProxy<MinPDPlugin>("MinPD", PluginManager::getInstance());
